...
const TRENDING_DATA: Record<CategoryId, TrendingItem[]> = {
    ...
    markets: [
        ...
        { id: "m4", name: "Marché de Noël de Bruges", location: "Bruges, Bruges", image: "https://images.unsplash.com/photo-1514190051997-0f6f39ca5cde?w=800&q=80", rating: 4.9, likes: 9876, bookings: 0, tag: "Romantique", price: "Entrée libre" },
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
                    <div>
                        ...
                        <h2 className="font-display text-2xl md:text-3xl lg:text-4xl font-bold text-foreground tracking-tight mt-3">
                            {SECTION_TITLES[category]}
                        </h2>
                        <p className="text-foreground-soft text-sm mt-2 max-w-[50ch]">
                            Choix de notre communauté WIGO cette semaine
                        </p>
                    </div>
                    <div className="flex items-center gap-2 bg-foreground/[0.03] border border-[var(--border)] rounded-full px-4 py-2">
                        <TrendingUp className="w-4 h-4 text-emerald-500" />
                        <span className="text-xs font-medium text-foreground-soft">Mis à jour en temps réel</span>
                    </div>
                </div>
                    ...
                    <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-3 mb-8">
                        {CATEGORIES.map(cat => (
                            <button key={cat.id} onClick={() => setLocalCategory(cat.id)}
                                className={`flex items-center gap-1.5 whitespace-nowrap px-4 py-2 rounded-full text-sm font-semibold border transition-all duration-200 active:scale-[0.97] ${localCategory === cat.id
                                    ? "bg-foreground text-background border-foreground"
                                    : "bg-transparent text-foreground/50 border-[var(--border)] hover:border-[var(--border-strong)] hover:text-foreground"
                                    }`}
                ...
                {/* Asymmetric Bento Grid — first card featured */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {items.map((item, i) => (
                        <article
                            key={item.id}
                            className={`group bg-card border border-[var(--border)] rounded-2xl overflow-hidden card-lift animate-slide-up ${
                                i === 0 ? "md:col-span-2 md:row-span-2" : ""
                            }`}
                            style={{ animationDelay: `${i * 0.05}s` }}
                        >
                            {/* Image */}
                            <div className={`relative overflow-hidden ${i === 0 ? "h-72 md:h-[400px]" : "h-52"}`}>
                                <Image
                                    src={item.image}
                                    alt={item.name}
                                    fill
                                ...
                                {/* Rank badge — refined */}
                                <div className="absolute top-4 left-4 w-9 h-9 rounded-xl bg-background/95 backdrop-blur-sm border border-[var(--border)] flex items-center justify-center font-bold text-sm text-foreground">
                                    {i + 1}
                                </div>
                                {/* Tag — subtle pill */}
                                <div className="absolute top-4 right-14 text-xs font-semibold px-3 py-1.5 rounded-full bg-foreground/80 text-background backdrop-blur-sm">
                                    {item.tag}
                                </div>
                                {/* Like button */}
                                <button onClick={() => toggleLike(item.id)}
                                    className={`absolute top-4 right-4 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 active:scale-[0.93] ${liked.has(item.id)
                                        ? "bg-red-500 text-white"
                                        : "bg-background/90 backdrop-blur-sm text-foreground/50 hover:text-red-500"
                                        }`}
                                    style={{ transitionTimingFunction: 'cubic-bezier(0.22, 1, 0.36, 1)' }}
                                >
                                ...
                                {item.price && (
                                    <div className="absolute bottom-4 left-4 bg-background/90 backdrop-blur-sm border border-[var(--border)] px-3 py-1.5 rounded-full">
                            ...
                            <div className="p-5">
                                <h3
                                    className={`font-bold text-foreground leading-tight mb-1 truncate transition-colors duration-200 ${
                                ...
                                <p className="text-sm text-foreground-soft mb-4">{item.location}</p>
                                ...
                                <div className="flex items-center justify-end gap-2">
                                    ...
                                    <Link href={`/hotel/${encodeURIComponent(item.id)}?name=${encodeURIComponent(item.name)}&location=${encodeURIComponent(item.location)}&img=${encodeURIComponent(item.image)}&price=${encodeURIComponent(item.price || "")}&rating=${item.rating}`}
                                        className="text-sm font-semibold text-accent flex items-center gap-1 hover-underline transition-colors duration-200 px-2"
                                        style={{ transitionTimingFunction: 'cubic-bezier(0.22, 1, 0.36, 1)' }}
                                    >
                                        Voir →
                                    </Link>
